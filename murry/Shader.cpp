...
Shader::Shader(const std::string& filename) {
	...
	CheckShaderError(m_program,GL_LINK_STATUS,true ,"Error: Program link failed: ");
...
GLuint Shader::CreateShader(const std::string& text, unsigned int shaderType) {
	...
	shaderSourceStringLens[0] = text.length();
	
	glShaderSource(shader,1,shaderSourceStrings,shaderSourceStringLens);
	glCompileShader(shader);
	
	CheckShaderError(shader,GL_COMPILE_STATUS,false, "Error: Program shader compile failed! ");
